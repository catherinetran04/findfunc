...
int main(int argc, char **argv){
  ...
  // could check here for ehdr->e_ident[EI_OSABI] for ELFOSABI_LINUX
  ...
  // .shstrtab. Find  its section index in the ELF header with the
  // field (e_shstrndx). Use the index into the array of section headers
  ...
  for(int i=0; i<symtab_num; i++){
    ...
    if(strcmp(func_name, cur_name) == 0){
      sym_st = &symtab[i];
    }
  }
